...
def match_anywhere(needles, haystack, ignore_case=False):
    ...
    def found(haystack): return re.search(
        regex_needle,
        haystack.path,
        flags=regex_flags,
    )
...
def match_consecutive(needles, haystack, ignore_case=False):
    ...
    regex_needle = regex_one_sep.join(
        imap(re.escape, needles)) + regex_no_sep_end
    regex_flags = re.IGNORECASE | re.UNICODE if ignore_case else re.UNICODE

    def found(entry): return re.search(
        regex_needle,
        entry.path,
        flags=regex_flags,
    )
...
def match_fuzzy(needles, haystack, ignore_case=False, threshold=0.6):
    ...
    if ignore_case:
        ...
        def match_percent(entry): return SequenceMatcher(
            a=needle,
